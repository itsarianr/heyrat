...
app.get('/api/search', (req, res) => {
  ...
  if (poetId) {
    poets = poets.filter(p => p.id === poetId);
  }

  const searchItems = [];
  poets.forEach(poet => {
    let books = poet.books;
    if (bookId) {
  ...
  const paginatedResults = searchItems.slice(startIndex, startIndex + pageSize);
